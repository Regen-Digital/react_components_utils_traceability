...
describe('Scanner', () => {
  ...
  test('should call closeQrCodeScanner when ref method is invoked', () => {
    // Render the component and set up the ref
    act(() => {
      render(<Scanner qrCodeSuccessCallback={() => {}} />);
    });

    const scannerRef = React.createRef<IScannerRef>();

    act(() => {
      render(<Scanner qrCodeSuccessCallback={() => {}} ref={scannerRef} />);
    });

    // Invoke the ref method
    act(() => {
      scannerRef.current?.closeQrCodeScanner();
    });

    expect(Html5Qrcode.prototype.stop).toHaveBeenCalledTimes(1);
  });
});
